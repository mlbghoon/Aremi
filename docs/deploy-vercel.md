...
> ✅ **배포 완료 (2026-07-03)** — 대표 주소: **https://aremi.vercel.app** · GitHub `main` 자동 배포.
...
- **카카오** 콘솔 → 앱 → **플랫폼 키** → 가운데 **JavaScript 키** 칸 아래 **`JS SDK 도메인`** 태그 클릭 → 배포 주소 추가
  ```
  https://<배포주소>      예) https://aremi.vercel.app
  ```
  > ⚠️ **헷갈리는 곳:** 왼쪽 메뉴의 **"웹 도메인"** 페이지는 여기가 아니다(그건 링크 공유용). 지도가 켜지려면
  > 반드시 **플랫폼 키 → JavaScript 키 → `JS SDK 도메인`** 에 넣어야 한다. 기존 `http://localhost` 는 지우지 말 것.
